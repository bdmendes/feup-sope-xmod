...
static int parse_symbolic_mode(const char *mode, XmodCommand *xmodCommand) {
    ...
    /*
    if(mode[1] == '-') xmodCommand->mode.symbolic_mode.permission_operator_type
    = SUBTRACT; else if(mode[1] == '+')
    xmodCommand->mode.symbolic_mode.permission_operator_type = ADD; else
    if(mode[1] == '=') xmodCommand->mode.symbolic_mode.permission_operator_type
    = ASSIGN; else return 1; //invalid command
    */

    xmodCommand->mode.symbolic_mode.permissions.read =
        strchr(mode, 'r') != NULL;
    xmodCommand->mode.symbolic_mode.permissions.write =
        strchr(mode, 'w') != NULL;
    xmodCommand->mode.symbolic_mode.permissions.execute =
        strchr(mode, 'x') != NULL;
